...
if [[ $1 == "uninstall" ]]; then
    ...
    VERSION=0.13.5
    if curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install.sh | bash -s -- -v $VERSION -p /usr/local --plugins wasi_nn-ggml wasmedge_rustls; then
